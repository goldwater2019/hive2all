...
public class File2HiveTest {
    ...
    public static void loadAndInsertIntoHive(SparkSession spark, String taskTableName, Map<String, String> configuration) {
        ...
        String str = configuration.get("output.dir");
        if (str == null) {
            outputDir = "output";
        } else {
            outputDir = str;
        }
        String ddlPath = MessageFormat.format("{0}/{1}/ddl",outputDir, taskTableName);
        String dataPath = MessageFormat.format("{0}/{1}/json", outputDir,taskTableName);
...
//        Row first = spark.sql("show create table edw_cdm.dim_tms_ane_lw_type;").javaRDD().first();
//        System.out.println(first.getString(0));
